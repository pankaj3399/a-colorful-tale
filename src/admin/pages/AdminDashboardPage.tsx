...
export default function AdminDashboardPage() {
  ...
  const activeFilterCount = [
    filters.search, filters.profileType, filters.classificationSource,
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      try {
        ...
        const message = getApiErrorMessage(dashboardError, 'Unable to load dashboard data.');
  ...
  function handleReset() { setFilters(initialFilters); }
  ...
  return (
    <DashboardLayout fill>
      <div className="flex min-h-0 flex-1 flex-col gap-6">
        <section className="shrink-0 animate-[fadeInUp_0.5s_ease-out]">
          <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-slate-600">Dashboard</p>
              <h1 className="mt-2 text-2xl font-semibold text-white sm:text-3xl">Analytics Overview</h1>
              <p className="mt-2 max-w-xl text-[13px] leading-relaxed text-slate-500">
                Monitor user intake, contact capture, tag distribution, and funnel progression from your internal workspace.
              </p>
            </div>
            <div className="flex items-center gap-2 self-start rounded-lg px-3 py-1.5 lg:self-auto"
