...
/** Flatten the API's `_id` + nested `capturedData` into the table's row shape. */
function normalizeAnalyticsRow(raw: RawAnalyticsRow): AnalyticsRow {
  return {
    id: raw._id ?? raw.id ?? '',
    userId: raw.userId ?? null,
    name: raw.name ?? null,
    email: raw.capturedData?.email ?? raw.email ?? null,
    phone: raw.capturedData?.phone ?? raw.phone ?? null,
    profileType: raw.profileType ?? null,
...
export async function fetchAnalyticsList(filters: AnalyticsFilters) {
  const response = await adminApiClient.get<RawAnalyticsListResponse>(
  ...
  const data: AnalyticsListResponse = {
    rows: (response.data.rows ?? []).map(normalizeAnalyticsRow),
