...
const extractAssistantText = (payload: unknown): string | null => {
  ...
  if (record.data && typeof record.data === 'object') {
    ...
    candidates.push(data.reply, data.response, data.message, data.assistantMessage, data.text);
  }
...
const extractInitialGreeting = (payload: unknown) => {
  ...
  if (!firstAssistantMessage || typeof firstAssistantMessage !== 'object') return null;
  const entry = firstAssistantMessage as Record<string, unknown>;
  return typeof entry.text === 'string' ? entry.text.trim() : null;
};

const bubbleCornerClass = (first: boolean, last: boolean, isAssistant: boolean) => {
  const side = isAssistant ? 'assistant' : 'user';
  if (first && last) return `cb-bubble-${side}-solo`;
  if (first) return `cb-bubble-${side}-first`;
  if (last) return `cb-bubble-${side}-last`;
  return `cb-bubble-${side}-middle`;
};

const Chatbot: React.FC<ChatbotProps> = ({ isFullscreen = false }) => {
    ...
    (text: string, delayMs: number, options?: { closeAfterReply?: boolean }) => {
      const timerId = window.setTimeout(() => {
        ...
        if (options?.closeAfterReply) {
          setConversation((current) => ({ ...current, isComplete: true }));
        }
        setIsWaitingForReply(false);
  ...
  const sendMessage = useCallback(
    async (rawMessage: string) => {
      ...
      try {
        ...
        if (assistantText) {
          deliverAssistantReply(assistantText, assistantDelayMs, { closeAfterReply });
  ...
  return (
    <div className={`chatbot-root${isFullscreen ? ' chatbot-root--hidden' : ''}`}>
      <AnimatePresence>
        {isOpen && (
          <motion.div
            key="panel"
            ...
            className="cb-panel"
          >
            <div aria-hidden="true" className="cb-ambient" />
            ...
            <div className="cb-panel-inner">
              ...
              <div className="cb-header">
                <div className="cb-header-left">
                  <div className="cb-avatar-wrap">
                    <img src={avatarImage} alt="AI-GENT 001" className="cb-avatar" />
                    <span className="cb-presence-dot" />
                  </div>
                  <div>
                    <p className="cb-header-name">AI-GENT 001</p>
              ...
              {/* Messages */}
              <div ref={scrollRef} className="cb-messages chatbot-thin-scrollbar">
                {messages.map((message, index) => {
                  ...
                  return (
                    <motion.div
                      ...
                      className={`cb-message-row ${isAssistant ? 'cb-message-row--assistant' : 'cb-message-row--user'} ${first ? 'cb-message-row--first' : 'cb-message-row--continued'}`}
                    >
                      {isAssistant && (
                        <div className="cb-message-avatar-slot">
                          {last && (
                            <img src={avatarImage} alt="" className="cb-message-avatar" />
                          )}
                        </div>
                      )}

                      <div
                        className={`cb-bubble-col ${isAssistant ? 'cb-bubble-col--assistant' : 'cb-bubble-col--user'}`}
                      >
                        <div
                          className={`cb-bubble ${
                            isAssistant
                              ...
                              : isFailed
                                ? 'cb-bubble--failed'
                                : 'cb-bubble--user'
                          } ${bubbleCornerClass(first, last, isAssistant)}`}
                        >
                          {message.text}
                        </div>
                        ...
                        {last && (
                          <div
                            className={`cb-meta ${isAssistant ? 'cb-meta--assistant' : 'cb-meta--user'}`}
                          >
                            <span>{formatMessageTime(message.timestamp)}</span>
                            {!isAssistant &&
                              (message.status === 'sending' ? (
                                <Clock size={9} className="cb-meta-icon cb-meta-icon--dim" />
                              ) : message.status === 'sent' ? (
                                <CheckCheck size={11} className="cb-meta-icon" />
                              ) : message.status === 'failed' ? (
                                <span className="cb-meta-failed">!</span>
                              ) : null)}
                          </div>
                        )}
                      </div>
              ...
              <div className="cb-input-area">
                ...
                >
                  <div className="chatbot-input-wrap cb-input-wrap">
                    <textarea
                      ref={textareaRef}
                      value={inputValue}
                      ...
                      placeholder={isClosed ? 'This conversation has ended.' : 'Message…'}
                      disabled={isClosed || isSending}
                      rows={1}
                      className="cb-textarea chatbot-thin-scrollbar"
                    />
                    <motion.button
                      type="submit"
                      disabled={!canSend}
                      ...
                      className={`chatbot-send-btn cb-send-btn ${canSend ? 'cb-send-btn--active' : 'cb-send-btn--idle'}`}
        ...
        {!isOpen && (
          ...
          >
            <span aria-hidden="true" className="cb-fab-sheen" />
            <motion.span
              aria-hidden="true"
              animate={{ scale: [1, 1.06, 1], opacity: [0.2, 0.4, 0.2] }}
              transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut' }}
              className="cb-fab-ring"
            />
            <div className="cb-fab-avatar-wrap">
              <img src={avatarImage} alt="AI-GENT 001" className="cb-fab-avatar" />
              <AnimatePresence>
                {unreadCount > 0 && (
                  <motion.span
            ...
            <div className="cb-fab-text">
              <p className="cb-fab-title">AI-GENT 001</p>
              <p className="cb-fab-subtitle">How may I help you?</p>
            </div>
          </motion.button>
        )}
      </AnimatePresence>
